...
- `HTTP_ADDR` - listen address, for example `:8080`
...
The repository includes a Dockerfile so the API can be built and deployed as a standard container image.

This makes the service portable across common platforms and keeps the first deployment path simple.
